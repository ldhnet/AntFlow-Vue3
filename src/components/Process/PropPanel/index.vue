  ...
  >
    ...
    >
      <el-tabs v-model="activeName" class="pane-tab">
        <el-tab-pane
          :label="'设置' + (value.type === 'approver' ? '审批人' : '发起人')"
          name="config"
        >
          <!-- 开始节点 -->
          ...
          <div class="form-auth-table">
            <header class="auth-table-header">
              <div class="row">
                ...
                >
                  <el-radio :label="2" style="margin-left: 1rem"
                    >可编辑</el-radio
                  >
            ...
            <div class="auth-table-body">
              ...
              >
                ...
                <el-radio-group v-model="item.formOperate" class="radio-group">
                  <el-radio :label="2" style="margin-left: 1rem"
                    ><span style="opacity: 0">可编辑</span></el-radio
                  >
                  <el-radio :label="1"
                    ><span style="opacity: 0">只读</span></el-radio
                  >
                  <el-radio :label="0"
                    ><span style="opacity: 0">隐藏</span></el-radio
                  >
                </el-radio-group>
              </div>
            </div>
...
export default {
  ...
  methods: {
    ...
    onOrgChange(data) {},
    timeTangeLabel(item) {
      const index = ["fc-time-duration", "fc-date-duration"].findIndex(
        (t) => t === item.tag
      );
      if (index > -1) {
    ...
    getAssignTypeLabel() {
      const res = this.assigneeTypeOptions.find(
        (t) => t.value === this.approverForm.assigneeType
      );
    ...
    initFormOperates(target) {
      ...
      const formItems = this.$store.state.formItemList.filter(
        (t) => t.cmpType !== "custom"
      );
      format(formItems);
      return res;
        ...
        .forEach((t) => {
          ...
          if (numberTypeCmp.includes(t.tag)) {
            if (cValue.type === "bet") {
              ...
              nodeContent +=
                `[${numVal[0]} ${rangeType[numVal[1]]} ${t.label} ${
                  rangeType[numVal[2]]
                } ${numVal[3]}] ` + "\n";
            } else {
              nodeContent +=
                `[${t.label} ${rangeType[cValue.type]} ${cValue.value}] ` +
                "\n";
            }
          } else if (t.tag === "fc-org-select") {
            const index = this.pconditions.findIndex(
              (p) => p.formId === t.formId
            );
    ...
    startNodeComfirm() {
      ...
      this.$emit(
        "confirm",
        this.properties,
        this.getOrgSelectLabel("start") || "所有人"
      );
      this.visible = false;
    },
    /**
    ...
    onDelCondition(condition) { 
      ...
      if (index > -1) {
        ...
        this.pconditions.find(
          (t) => t.formId === condition.formId
    ...
    initInitiator() {
      const initiator =
        this.value.properties && this.value.properties.initiator;
    ...
    initApproverNodeData() {
      ...
      if (Array.isArray(this.approverForm.approvers)) {
        if ("user" === this.approverForm.assigneeType) {
          this.approverUserIds = approvers.map((c) => c.userId);
        } else {
