...
export class NodeUtils {
  ...
  /**
   * 获取金额比较类型
   * @param { String } compareType - 节点数据
   * @returns Int
   */
  static getCompareTypeInt(compareType) {
    switch (compareType) {
      case "eq":
        return 5;
      case "lt":
  ...
  /**
   * 获取金额比较类型
   * @param { Int} compareType - 节点数据
   * @returns String
   */
  static getCompareTypeString(compareType) {
    switch (compareType) {
      case 5:
        return "eq";
      case 4:
  ...
  static createNodeExtend(type, nodeData) {
    ...
    if (!isEmptyArray(nodeData.conditionNodes)) {
      prevIds = nodeData.conditionNodes.map((item) => {
        return item.nodeId;
      });
    } else {
  ...
  static depthMapNodeTree(currentNode, resultData) {
    ...
    if (
      isEmpty(currentNode.childNode) &&
      isEmptyArray(currentNode.conditionNodes)
    ) {
  ...
  static deleteNode(nodeData, processData, checkEmpty = true) {
    ...
    let v1_updateChildNodeForGateway = (delGatewayNode, processData) => {
      let prevForGateway = this.getPreviousNode(
        delGatewayNode.prevId,
        processData
      );
    ...
    let updateChildNodeFrom = (prev, delNode) => {
      prev.nodeTo = prev.nodeTo.filter((m) => {
        return m != delNode.nodeId;
      });
      for (let i in delNode.nodeTo) {
        let childNodeInfo = this.getPreviousNode(
          delNode.nodeTo[i],
          processData
        );
        if (isEmpty(childNodeInfo)) return;
        childNodeInfo.nodeFrom = childNodeInfo.nodeFrom.filter((key) => {
          return key != delNode.nodeId;
        });
      }
      //console.log('delete=====processData====',JSON.stringify(processData));
    };
    ...
    let updateGatewayChildNode = (prev, delNode) => {
      ...
      if (!isEmpty(delNode.childNode)) {
        ...
        delNode.childNode.nodeFrom = allConditionNode.map((c) => {
          return c.nodeId;
        });
        prev.childNode = delNode.childNode;
      } else {
    ...
    let V2_concatChild = (deletePrev, delNode) => {
      ...
      if (prev.type === "gateway") {
        ...
        lastChildNodes = lastChildNodes.map(
          (c) => (c.nodeTo = [prev.childNode.nodeId])
        ); //改变原父节点 nodeTo 数据
        prev.childNode.nodeFrom = lastChildNodes.map((c) => {
          return c.nodeId;
        });
      } else {
  ...
  static addApprovalNode(data, isBranchAction, newChildNode = undefined) {
    ...
    if (data.type == "gateway" && !isEmptyArray(data.conditionNodes)) {
      ...
      newChildNode.nodeFrom = lastNodes.map((c) => {
        return c.nodeId;
      }); //新节点 nodeFrom赋值
  ...
  static addCopyNode(data, isBranchAction) {
    ...
    this.addApprovalNode(
      data,
      isBranchAction,
      this.createNode("copy", data.nodeId)
    );
  }
  /**
  ...
  static appendConditionNode(data) {
    ...
    let defaultNodeIndex = conditions.findIndex(
      (node) => node.properties.isDefault
    );
    ...
    if (defaultNodeIndex > -1) {
      ...
      conditions[conditions.length - 1].properties.priority =
        conditions.length - 1;
    } else {
    ...
    if (!isEmptyArray(lastNodes[0].nodeTo)) {
      if (!isEmpty(data.childNode)) {
        data.childNode.nodeFrom = lastNodes.map((c) => {
          return c.nodeId;
        });
      }
  ...
  static appendBranch(data, isBottomBtnOfBranch) {
    ...
    if (!isEmpty(gatewayNode.childNode)) {
      let nodeToIds = conditionNodes.map((c) => {
        return c.nodeId;
      });
  ...
  static increasePriority(data, processData) {
    ...
    if (index) {
      ...
      branchData[index - 1] = branchData.splice(
        index,
        1,
        branchData[index - 1]
      )[0];
  ...
  static setDefaultCondition(cnode, processData) {
    ...
    const hasCondition = (node) =>
      node.properties &&
      (node.properties.initiator || !isEmptyArray(node.properties.conditions));
    ...
    count > 0 && !hasCondition(lastNode)
      ? setDefault(lastNode)
      : clearDefault(lastNode);
  }
  /**
   * 校验单个节点必填项完整性
   * @param {Node} node - 节点数据
